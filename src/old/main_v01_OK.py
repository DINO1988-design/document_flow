...
def save_document(tipo_documento, file_path, fields):
    """Salva documento e campi nel database"""
    with engine.begin() as conn:
        ...
        for campo, valore in fields.items():
            # converti dict/list in JSON per evitare troncamenti
...
def parse_data_with_gpt(text, tipo_documento="generico"):
    ...
    try:
        ...
        - Campi possibili: nome, cognome, data_nascita, codice_fiscale, indirizzo, telefono, email,
          numero_prenotazione, numero_biglietto, data_partenza, orario_partenza, porto_partenza,
          porto_arrivo, durata_viaggio, dettagli_passeggeri (lista), tipo_cabina, importo_totale
        - Per liste o oggetti complessi, restituisci JSON annidato.
        - Riporta tutti i dati presenti nel testo, anche non standard.
        Testo OCR:
        \"\"\"{text}\"\"\"
        """
...
def generate_word(fields, original_file):
    ...
    for campo, valore in fields.items():
        if isinstance(valore, (dict, list)):
            valore_str = json.dumps(valore, ensure_ascii=False, indent=2)
        else:
            valore_str = str(valore)
        doc.add_paragraph(f"{campo}: {valore_str}")
